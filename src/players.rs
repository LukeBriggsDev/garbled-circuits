use aes_gcm::{AeadCore, AeadInPlace, Aes256Gcm, Key, KeyInit};
use aes_gcm::aead::{Aead, Nonce, OsRng};
use itertools::iproduct;

pub struct AESNoncePair {
    key: Key<Aes256Gcm>,
    nonce: Nonce<Aes256Gcm>
}
impl AESNoncePair {
    fn new() -> AESNoncePair {
        AESNoncePair {
            key: Aes256Gcm::generate_key(OsRng),
            nonce: Aes256Gcm::generate_nonce(&mut OsRng)
        }
    }
}

pub struct GarblerKeyPair {
    zero_key: AESNoncePair,
    one_key: AESNoncePair
}
impl GarblerKeyPair {
    fn new() -> GarblerKeyPair {
        GarblerKeyPair {
            zero_key: AESNoncePair::new(),
            one_key: AESNoncePair::new(),
        }
    }
    fn get(&self, bit: u8) -> &AESNoncePair {
        if (bit == 1) {
            &self.one_key
        } else {
            &self.zero_key
        }
    }
}

pub struct Garbler {
    outputs: Vec<Vec<u8>>,
    keys: Vec<GarblerKeyPair>,
    choice: u8,
    input: GarblerKeyPair

}
impl Garbler {
    pub fn new() -> Garbler {
        Garbler {
            outputs: vec![],
            keys: vec![],
            choice: 0,
            input: GarblerKeyPair::new(),
        }
    }
    fn generate_keys(&mut self) {
        self.keys = Vec::new();
        for _ in 0..2 {
            self.keys.push(GarblerKeyPair::new())
        }
    }
    pub(crate) fn garble(&mut self) {
        self.generate_keys();
        let inputs = iproduct!(vec![0u8, 1u8].into_iter(), vec![0u8, 1u8].into_iter());
        self.outputs = Vec::new();
        for input in inputs {
            match input {
                (a, b) => {
                    let mut ciphertext = Vec::new();
                    ciphertext.push(a & b);
                    let key: Vec<u8> = self.keys[0].get(a).key.as_slice().iter()
                        .zip(self.keys[1].get(b).key.as_slice().iter())
                        .map(|(&key1, &key2)| key1 ^ key2)
                        .collect();

                    let cipher = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key.as_slice()));
                    cipher.encrypt_in_place(&self.keys[0].get(0).nonce, b"", &mut ciphertext).unwrap();
                    self.outputs.push(ciphertext);
                }
            }
        }
        for output in &self.outputs {
            let key: Vec<u8> = self.keys[0].get(1).key.as_slice().iter()
                .zip(self.keys[1].get(1).key.as_slice().iter())
                .map(|(&key1, &key2)| key1 ^ key2)
                .collect();
            let cipher_bob = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key.as_slice()));
            let decrypt_a = cipher_bob.decrypt(&self.keys[0].get(0).nonce, output.as_slice());
            match decrypt_a {
                Ok(message) => {
                    println!("{:?}", message.as_slice())
                }
                Err(message) => {
                    println!("{}", message)
                }
            }
        }
    }
}

pub struct Evaluator {
    choice: u8,
    key_pair: GarblerKeyPair
}
