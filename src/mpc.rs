...
fn oblivious(message_a: Vec<u8>, message_b: Vec<u8>) {
    // Alice part a
...
impl OTEval for Circuit {
    fn ot_eval(&self, inputs: &[Vec<bool>]) -> Vec<bool> {
        ...
        for _ in 0..2 {
            a_keys.push(Aes256Gcm::generate_key(OsRng));
        }
        let mut b_keys = Vec::new();
        for _ in 0..2 {
            b_keys.push(Aes256Gcm::generate_key(OsRng));
        }
        ...
        for output in outputs {
            let key: Vec<u8> = a_keys[1 as usize].as_slice().iter()
                .zip(b_keys[1 as usize].as_slice().iter())
                .map(|(&key1, &key2)| key1 ^ key2)
                .collect();
            let cipher_bob = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key.as_slice()));
            let decrypt_a = cipher_bob.decrypt(&nonce, output.as_slice());
            match decrypt_a {
                Ok(message) => {
